...
def export_latex_format(df, directory):
    ...
    for method, method_df in grouped:
        ...
        file_path = os.path.join(directory, f'{method}-scenes_pca-BA.dat')
...
def main(directory, seg_dir, seg_prefix='rgb'):
    ...
    # Reorder columns
    columns_order = ['method', 'n_segs_param'] + \
        [col for col in df.columns if col not in ['method', 'n_segs_param']]
    df = df[columns_order]
    ...
    for i, (method, method_df) in enumerate(df.groupby('method')):
        ...
        plt.plot(method_df['n_segs_param'], method_df['bal_acc'],
                 label=method, linestyle=line_style)
    ...
    method_colors = {method: plt.cm.jet(
        i/len(unique_methods)) for i, method in enumerate(unique_methods)}
    method_markers = {method: random.choice(
        ['o', 's', '^', 'x', 'D', 'P', '*']) for method in unique_methods}

    # Plot each method separately
    for method, method_df in df.groupby('method'):
        plt.scatter(method_df['n_segs_param'], method_df['bal_acc'], label=method,
